...
const SolicitudesAprobacionComercial = ({ propuesta, usuarioId }) => {
    ...
    <div className="mx-auto px-4 sm:px-6 lg:px-12">
      <div className="bg-white shadow-soft p-6 mb-8 border border-gray-200 rounded-lg">
        {isLoading ? (
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2 mt-4">Solicitudes de Aprobacion Comercial</h3>
            {solicitudesGenerales.filter(s => s.tipo_solicitud === 'APROBACION_COMERCIAL').length === 0 ? (
              <div className="text-center py-4 text-gray-500">No hay solicitudes de aprobacion comercial.</div>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {solicitudesGenerales
                      .filter(s => s.tipo_solicitud === 'APROBACION_COMERCIAL')
                      .map(solicitud => (
      ...
      <ConfirmModal
        open={modal.open}
        mensaje={modal.accion === 'ACEPTAR' ? 'Esta seguro que desea aceptar esta solicitud?' : 'Esta seguro que desea rechazar esta solicitud?'}
...
export default SolicitudesAprobacionComercial;
