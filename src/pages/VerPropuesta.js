...
const VerPropuesta = () => {
  ...
  const getCarterasPermitidas = () => {
    if (currentRole === 'JP' && currentUserJP) {
      return currentUserJP.carteras;
    }
    if (currentRole === 'Subdirector' && currentUserSubdirector) {
      return currentUserSubdirector.carteras;
    }
    // DAF y Administrador ven todas las carteras
    return propuesta.carteras;
  };

  const carterasPermitidas = getCarterasPermitidas();
  const programasFiltrados = programas.filter(p => 
    propuesta.carteras.includes(p.cartera) && carterasPermitidas.includes(p.cartera)
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background-light via-background-subtle to-white">
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Vista de Propuesta</h1>
                <p className="text-gray-600 text-sm">Modo de solo lectura - {currentRole}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="w-[99vw] box-border mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-soft p-8 mb-8 border border-gray-100">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">Detalles y configuración actual (solo lectura)</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-900">{propuesta.estado}</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-900">{currentRole}</p>
            </div>
            
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">Carteras Visibles</label>
              <p className="text-sm text-gray-900">{carterasPermitidas.join(', ')}</p>
            </div>
          </div>
        </div>
        ...
        {/* Grilla de programas */}
        <div className="bg-white rounded-2xl shadow-soft p-8 mb-8 border border-gray-100">
          <h3 className="text-xl font-bold text-gray-800 mb-6">Lista de Programas (Solo Lectura)</h3>
                                       ...
                                       <tr key={m.identificador} className={
                                        ...
                                        <td className="px-2 py-1">
                                          ...
                                          {m.monto_editado_en_sesion && (
                                            <span className="text-xs bg-purple-200 text-purple-800 px-2 py-1 rounded ml-1">Monto editado</span>
                                          )}
                                        </td>
                                      </tr>
                                    ))
